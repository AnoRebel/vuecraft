...
async function handleEmailAuth() {
  if (mode.value === 'login') {
    ...
    const success = await registerWithEmail(email.value, password.value, name.value)
    if (success) {
      emit('update:open', false)
      resetForm()
    }
    ...
    <DialogContent class="max-w-sm">
      ...
      <div class="space-y-4 mt-4">
        ...
        <div class="space-y-2">
          ...
          <Button variant="outline" class="w-full" :disabled="isLoading" @click="loginWithGitHub">
        ...
        <p class="text-center text-sm text-muted-foreground">
          ...
          <button type="button" class="text-primary hover:underline ml-1" @click="switchMode">
            {{ mode === 'login' ? 'Sign up' : 'Sign in' }}
          </button>
        </p>

        <p class="text-xs text-center text-muted-foreground">Demo: demo@vuecraft.dev / demo123</p>
      </div>
    </DialogContent>
  </Dialog>
</template>
